    ...
    try {
        ...
        const session = await index_1.prisma.session.create({
            data: {
                ...
                gameType: 'None',
                playerCount: 1,
                staffUsername: staffUsername.toLowerCase(),
                startTime: now,
                status: 'active',
                payments: hasAdvance ? {
                    create: {
        ...
        (0, websocket_1.broadcast)({ type: 'SESSION_START', session });
...
router.get('/completed', auth_1.authenticateToken, async (req, res) => {
    try {
...
router.get('/customers/all', auth_1.authenticateToken, async (req, res) => {
    try {
        const customers = await index_1.prisma.session.findMany({
            where: {
                customerName: { not: '' }
            },
...
router.get('/:id', auth_1.authenticateToken, async (req, res) => {
    try {
        ...
        const priorUdhars = await index_1.prisma.udhar.findMany({
            where: {
...
router.post('/:id/table/start', auth_1.authenticateToken, async (req, res) => {
    ...
    try {
        ...
        if (needsTable) {
            ...
            table = await index_1.prisma.table.findUnique({ where: { id: parseInt(tableId) } });
            if (!table) {
        ...
        const [_, __, play] = await index_1.prisma.$transaction([
            ...(table ? [index_1.prisma.table.update({
                    where: { id: table.id },
                    data: { status: 'occupied' }
                })] : []),
            index_1.prisma.session.update({
                where: { id: sessionId },
                data: {
            ...
            index_1.prisma.tablePlay.create({
                data: {
                    sessionId,
        ...
        (0, websocket_1.broadcast)({ type: 'TABLE_PLAY_START', play });
...
router.post('/:id/table/end', auth_1.authenticateToken, async (req, res) => {
    const sessionId = req.params.id;
    ...
    try {
        ...
        const [_, play, __] = await index_1.prisma.$transaction([
            ...(activePlay.tableId ? [index_1.prisma.table.update({
                    where: { id: activePlay.tableId },
                    data: { status: 'available' }
                })] : []),
            index_1.prisma.tablePlay.update({
        ...
        (0, websocket_1.broadcast)({ type: 'TABLE_PLAY_END', play });
        res.json(play);
    }
    catch (error) {
        res.status(500).json({ error: error.message });
...
router.post('/:id/close', auth_1.authenticateToken, async (req, res) => {
    ...
    try {
        ...
        const activePlays = await index_1.prisma.tablePlay.findMany({
        ...
        const closedSession = await index_1.prisma.$transaction(async (tx) => {
        ...
        (0, websocket_1.broadcast)({ type: 'SESSION_CLOSE', session: closedSession });
...
router.post('/:id/send-whatsapp', auth_1.authenticateToken, async (req, res) => {
    const { id } = req.params;
    try {
        const session = await index_1.prisma.session.findUnique({
            where: { id },
